...
import { getCachedDoc, seedDocCache } from './firestore.js?v=151';
...
function showEventModal(event) {
    ...
    const checkInWindow = getCheckInWindow(event.time);

    title.textContent = event.category;
    if (checkin) checkin.textContent = checkInWindow ? `Check In: ${checkInWindow}` : '';
    time.textContent = `Tee Time: ${formatTime12Hour(event.time)}`;
